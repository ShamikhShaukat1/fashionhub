...
<aside class="fixed left-0 top-0 z-40 w-64 h-screen bg-stone-900 border-r border-stone-800 flex flex-col">
    ...
    <nav class="flex-1 p-4 space-y-1 overflow-y-auto">
        @foreach ($menuItems as $item)
            @php
                $isActive = request()->routeIs($item['route']) || request()->routeIs($item['active_pattern']);
            @endphp
    ...
    <div class="p-4 border-t border-stone-800">
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit"
                class="w-full px-4 py-2.5 rounded-xl bg-stone-800 hover:bg-stone-700 text-stone-300 hover:text-white text-xs uppercase tracking-wider font-semibold transition">
                Sign Out
            </button>
        </form>
